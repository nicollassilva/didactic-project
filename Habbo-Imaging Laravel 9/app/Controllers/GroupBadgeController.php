...
class GroupBadgeController extends Controller
{
    ...
    {
        $Dir = public_path('habbo\imaging\badgeparts');

        define('BASECOLORSRGB', [
            1  =>  [255, 214, 1],
            2  =>  [236, 118, 0],
            3  =>  [132, 222, 0],
            4  =>  [88, 154, 0],
        ...
        define('UNPAINTABLE', [209, 210, 211, 212]);
        ...
        // Security
        if (!isset($badge) || CheckStr($badge))
            exit;

        $BadgeCode = str_replace(['.gif', 'X'], '', $badge);
        // Cache
        if (File::exists($Dir . '/cache/' . $BadgeCode . '.gif')) {
        ...
        if ($Base) {
            ...
            if (File::exists($Dir . '/base/' .  $BasePartArray[0] . '_' .  $BasePartArray[0] . '.gif'))
        ...
        if ($Symbols) {
            foreach ($SymbolsCode as $SymbolCode) {
                ...
                if ($SymbolPartArray[0] == 0 || !File::exists($Dir . '/templates/' . $SymbolPartArray[0] . '.gif')) {
                    continue;
                }
                ...
                if (File::exists($Dir . '/templates/' . $SymbolPartArray[0] . '_' . $SymbolPartArray[0] . '.gif'))
